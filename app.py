# app.py
# =========================================================
# 📈 STOCK PROJECTION SIMULATOR — Streamlit Dashboard (v4.3)
...
with st.sidebar:
    ...
    # fetch suggestions only when >=2 chars (prevents flicker)
...
if run:
    ...
    # daily log returns from 5Y history
    ...
    for name, mult in scenarios.items():
        path = mc_paths(last, mu, sig, mc_days, n_paths,
                        daily_yield=daily_yield, drift_mult=mult, rng=rng)
        paths[name] = {
    ...
    with tab2:
        ...
        # approx quarterly payment dates
        pay_dates = []
        ...
        if pay_dates and last_div_amt > 0:
            ...
            if rows:
                ...
                # quarterly summary + yield on cost
                q = df_div.copy()
                ...
                st.dataframe(qsum.style.format({
                    "Total Payment ($)": "${:,.2f}",
                    "Contributions ($)": "${:,.2f}",
